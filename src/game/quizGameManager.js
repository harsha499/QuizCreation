 ...
 *
 * ROOT-CAUSE FIX #1 — Different questions on each browser:
 *   The old code built `const QS = ...` at MODULE LEVEL using Math.random().
 *   Every Node.js require() of this file reused the same shuffled array, but
 *   more importantly the CLIENT was ALSO shuffling locally with its own
 *   Math.random() — so browser A and browser B always got different orders.
 *
 *   Fix: QS is built INSIDE defGame() so each game room gets its own
 *   independently-shuffled list stored on the game object.  The client
 *   NEVER shuffles; it only renders what the server sends in state.question.
 *
 * ROOT-CAUSE FIX #2 — Steal message named the wrong team:
 *   Old code flipped currentTeam BEFORE building result.message, so the
 *   "gets a steal" message referred to the team that just answered wrong.
 *   Fix: capture names before the flip.
 *
 * ROOT-CAUSE FIX #3 — getQuestion sent the answer index to the client:
 *   Clients could read state.question.ans in devtools and cheat.
 *   Fix: getQuestion strips `ans` before returning.
 *
 * ROOT-CAUSE FIX #4 — restartGame didn't reset status to 'playing':
 *   After restart both browsers stayed on setupScreen.
 *   Fix: restartGame copies names and immediately sets status:'playing'.
 */
...
/**
 * shuffleQ — deterministic per-question OPTION shuffle seeded by question text.
 * Same seed → same order every time → both browsers see identical A/B/C/D letters.
 */
function shuffleQ(q) {
  let seed = 0;
...
/**
 * buildQS — FIX #1: called inside defGame(), not at module load.
 * Each game room shuffles question ORDER independently with Math.random(),
 * while option ORDER stays deterministic via shuffleQ.
 */
function buildQS() {
  return [1, 2, 3, 4, 5].flatMap(round => {
    const bucket = RAW.filter(q => q.r === round);
    // Fisher-Yates on a copy so RAW is never mutated
    const copy = [...bucket];
...
function defGame(teamA = "Alpha", teamB = WAITING_TEAM) {
  const qs = buildQS();             // ← FIX #1: per-game question list
  return {
    names:       [teamA, teamB],
    scores:      [0, 0],
    correct:     [0, 0],
    ...
    screen:      "setupScreen",
    status:      "waiting",
    qs,                               // stored on game object, not module-level
    total:       qs.length,
  };
}
...
/**
 * getQuestion — FIX #3: strips the `ans` field before sending to client.
 * Clients can't cheat by reading state.question.ans in devtools.
 */
function getQuestion(code) {
  const game = games[code];
  if (!game || game.qIndex >= game.total) return null;
  const { ans: _hidden, ...safe } = game.qs[game.qIndex];   // strip answer
  return safe;
}
...
function handleAnswer(code, chosen) {
  ...
  // Guard: both teams already answered this question
  if (game.answered && game.stealUsed) return null;
  ...
  if (chosen === q.ans) {
    // ── Correct answer ────────────────────────────────────────────────────────
    // FIX #2: capture name BEFORE flipping currentTeam for the message
    ...
    game.currentTeam = 1 - game.currentTeam;   // winner goes first next round
    game.answered    = true;
    game.stealUsed   = true;
    result.correct   = true;
    result.pts       = pos;
    ...
    if (!game.stealUsed) {
      // First wrong — give the other team a steal attempt
      game.stealUsed   = true;
      game.currentTeam = 1 - game.currentTeam;   // switch to stealer
      game.answered    = false;                   // stealer still needs to answer
      result.status    = "steal";
      result.pts       = -neg;
      result.message   = `✗ Wrong! ${wrongName} −${neg} pts. ${stealerName} gets a steal chance!`;
    } else {
      // Steal also wrong — both missed, move on
      game.answered  = true;
      result.next    = true;
      result.status  = "miss";
      result.pts     = -neg;
...
// FIX #4: restartGame preserves names and immediately sets status:'playing'
...
/**
 * getState — everything the UI needs.
 * question is included (answer stripped by getQuestion).
 * isRoundBoundary tells the client to show the transition overlay.
 */
function getState(code) {
  ...
  // Signal round boundary: first Q of rounds 2-5 (qIndex 10,20,30,40)
  ...
  return {
    ...
    screen:          game.screen,
    status:          game.status,
    question,           // ← rendered by client; answer NOT included
    roundMeta,
    isRoundBoundary,
    total:           game.total,
  };
...
module.exports = { createGame, joinGame, startGame, handleAnswer, nextQuestion, restartGame, getState };