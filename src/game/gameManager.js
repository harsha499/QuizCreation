 ...
 *
 * BUG 1 ROOT-CAUSE FIX — questions skipping every other one:
 *   Both browsers render the "Next Question" button once a question is
 *   resolved, and both are wired to call nextQuestion(). With no guard,
 *   TWO separate 'nextQuestion' socket events arrive for the SAME resolved
 *   question — the first correctly advances qIndex by 1, the second
 *   advances it AGAIN before anyone has even seen the new question,
 *   silently skipping it. That produced the "every alternate question
 *   disappears" symptom.
 *
 *   Fix: nextQuestion() now requires game.answered === true to advance
 *   (i.e. the question must actually be resolved), and IMMEDIATELY flips
 *   answered back to false as part of the SAME synchronous call that
 *   increments qIndex. Because Node.js handles socket events one at a
 *   time on a single thread, the second 'nextQuestion' call — even if it
 *   arrives microseconds later — will see answered === false (already
 *   reset by the first call) and is rejected by the guard below, so it
 *   cannot advance qIndex a second time.
 */
function nextQuestion(code) {
  const game = games[code];
  if (!game) return null;

  // BUG 1 FIX: reject duplicate/late nextQuestion calls for the same question.
  // Only a question that is genuinely resolved (answered === true) can be
  // advanced. Once we advance, we flip answered to false in this same call,
  // so a second click — from the other browser — finds answered === false
  // and is turned away before it can touch qIndex.
  if (!game.answered) return null;
